CREATE TABLE CUSTOMERS(
  ...
  ,CustomerName VARCHAR(37) NOT NULL
  ,ContactName  VARCHAR(21) NOT NULL
...
create function add_int(a Integer, b Integer)
 returns Integer
 language plpgsql
 as $$
  begin 
...
CREATE FUNCTION getcountries()
 RETURNS VARCHAR[]
 LANGUAGE sql
 AS 'select array_agg(distinct country) from customers';

CREATE TYPE countrybasedcustomers AS (country varchar(12), customers varchar[]);
CREATE FUNCTION getcountrybasedcustomers()
 RETURNS countrybasedcustomers[]
 LANGUAGE plpgsql
AS $$
declare
    c varchar(12);
    tmp countrybasedcustomers;
    res countrybasedcustomers[] = array[]::countrybasedcustomers[];
begin
    for c in select distinct country from customers order by country
    loop
  select c, (select array_agg(customername) from customers where country = c) into tmp;
